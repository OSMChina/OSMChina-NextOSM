...
function getBaiduBounds() {
  ...
  let northEast = conv.gps84_To_bd09(bounds._northEast.lng, bounds._northEast.lat);
  let southWest = conv.gps84_To_bd09(bounds._southWest.lng, bounds._southWest.lat);
...
function getScript(url) {
  ...
  el.src = url;
  el.addEventListener('load', function (hX) {
    var hW = hX.target;
...
function searchPoi(name) {
  ...
  function getCity() {
    getScript(`https://api.map.baidu.com/?qt=cen&b=${getBaiduBounds()}&l=11&ie=utf-8&oue=1` +
      `&fromproduct=jsapi&res=api&callback=cityCallback&ak=E4805d16520de693a3fe707cdc962045&v=3.0`);
  }

  function getPoi(name, city) {
    getScript(`https://api.map.baidu.com/?qt=bd&c=${encodeURIComponent(city)}&wd=${encodeURIComponent(name)}&ar=(${getBaiduBounds()})&rn=10&l=18&ie=utf-8&oue=1` +
  ...
  window.poiCallback = function(result) {
    for (const item of result.content) {
      if (item.navi_x != 0 && item.navi_y != 0) {
        ...
        ${item.location.lat.toFixed(7)},${item.location.lng.toFixed(7)}<br>${item.addr}`)
