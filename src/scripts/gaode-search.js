...
function getAmapBounds() {
  ...
  let northEast = conv.gps84_To_gcj02(
    bounds._northEast.lng,
    bounds._northEast.lat
  );
  let southWest = conv.gps84_To_gcj02(
...
async function searchPoi(name, jsonTemplate, page) {
  ...
  const data = await fetch(
    `https://restapi.amap.com/v3/place/polygon?polygon=${getAmapBounds()}` +
  ...
  for (const item of data.pois) {
    ...
    if (jsonTemplate) {
      ...
      level0.push(
        `node: ${item.location.lat.toFixed(7)},${item.location.lng.toFixed(
          7
        )}\n` +
          Object.entries(jsonTemplate)
            .map((x) => `  ${x[0]} = ${x[1]}\n`)
            .join("")
      );
    }
